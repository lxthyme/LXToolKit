...
class PresentrController: UIPresentationController {
    deinit {
    ...
    fileprivate var conformingPresentedController: PresentrDelegate? {
        if let nav = presentedViewController as? UINavigationController,
           let visibleVC = nav.visibleViewController as? PresentrDelegate {
            return visibleVC
        }
    ...
    fileprivate var containerFrame: CGRect {
        return contextFrameForPresentation ?? containerView?.bounds ?? .zero
    }
    ...
    fileprivate lazy var shouldSwiperTop: Bool = {
        let defaultDirection = dismissOnSwipeDirection == .default
...
extension PresentrController {
    override var frameOfPresentedViewInContainerView: CGRect {
        ...
        let size = size(forChildContentContainer: presentedViewController, withParentContainerSize: containerBounds.size)

        let origin: CGPoint
        if let center = getCenterPointFromType() {
            origin = calculateOrigin(center, size: size)
        } else {
            origin = getOriginFromType() ?? CGPoint.zero
        }
...
// MARK: - 🔐
private extension PresentrController {
    func setupDismissOnSwipe() {
        let swipe = UIPanGestureRecognizer(target: self, action: #selector(presentedViewSwipe(gesture:)))
        presentedViewController.view.addGestureRecognizer(swipe)
    }
    func setupBackground(_ backgroundColor: UIColor, backgroundOpacity: Float, blurBackground: Bool, blurStyle: UIBlurEffect.Style) {
        let tap = UITapGestureRecognizer(target: self, action: #selector(chromeViewTapped(gesture:)))
        ...
        if blurBackground {
            ...
            chromeView.backgroundColor = backgroundColor.withAlphaComponent(backgroundOpacity.cgFloat)
        }
    }

    func setupCornerRadius(roundCorners: Bool?, cornerRadius: CGFloat) {
    ...
    func addDropShadow(shadow: PresentrShadow?) {
...
private extension PresentrController {
    ...
    @objc func presentedViewSwipe(gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .began {
            ...
            if (shouldSwipeBottom && directionDown) ||
                (shouldSwiperTop && !directionDown) {
    ...
    func swipeGestureChanged(gesture: UIPanGestureRecognizer) {
        ...
        let dismiss = shouldSwiperTop ? (amount.y < swipeLimit) : (amount.y > swipeLimit)
    ...
    func getWidthFromType(_ parentSize: CGSize) -> Float {
        guard let size = presentationType.size() else {
            if case .dynamic = presentationType {
                ...
                return presentedViewController.view.systemLayoutSizeFitting(sizeKey).width.float
            }
    ...
    func getHeightFromType(_ parentSize: CGSize) -> Float {
        guard let size = presentationType.size() else {
            if case .dynamic = presentationType {
                ...
                return presentedViewController.view.systemLayoutSizeFitting(sizeKey).height.float
            }
            return 0
        }
...
// MARK: - ✈️UIAdaptivePresentationControllerDelegate
// extension PresentrController: UIAdaptivePresentationControllerDelegate {
