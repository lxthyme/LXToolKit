...
@implementation LXJXCategoryCustomView
...
- (Class)preferredCellClass {
    return [LXJXCategoryCustomCell class];
}

- (void)refreshDataSource {
    NSMutableArray *tempArray = [NSMutableArray array];
    for (int i = 0; i < self.titles.count; i++) {
        LXJXCategoryCustomCellModel *cellModel = [[LXJXCategoryCustomCellModel alloc] init];
...
- (void)refreshCellModel:(JXCategoryBaseCellModel *)cellModel index:(NSInteger)index {
    ...
    LXJXCategoryCustomCellModel *myModel = (LXJXCategoryCustomCellModel *)cellModel;
