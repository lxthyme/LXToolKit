...
@property(nonatomic, strong)NSArray<NSArray<LXGoodBaseItemModel *> *> *goodsList;
/// <#label#>
...
- (void)formatGoodsList:(BOOL)showAll {
    ...
    if(self.classifyType == LXClassifyGoodItemTypeB2C) {
        self.goodsList = @[self.rightModel.f_goodsInfoModel.f_b2CGoodsListModel.goodsInfoList];
    } else if(self.classifyType == LXClassifyGoodItemTypeO2O) {
        NSMutableArray *goodsList = [NSMutableArray array];
        if(self.rightModel.f_shouldShowBanner) {
            LXGoodBaseItemModel *bannerItem = [[LXGoodBaseItemModel alloc]init];
            bannerItem.f_itemType = LXClassifyGoodItemTypeBanner;
            bannerItem.f_cellHeight = kBannerSectionHeight;
            [goodsList addObject:@[bannerItem]];
        }
        DJGoodsIdsModel *idsModel = self.rightModel.f_goodsInfoModel.f_o2oGoodsInfoList.firstObject;
        if(showAll && self.rightModel.f_showAll && [idsModel.cateId isEqualToString:@"-1"]) {
            self.isAll = YES;
            [goodsList addObject:idsModel.f_goodsList];
        } else {
            self.isAll = NO;
            NSArray *tmp = [self.rightModel.f_goodsInfoModel.f_o2oGoodsInfoList.rac_sequence map:^id(DJGoodsIdsModel *value) {
                return value.f_goodsList;
            }].array;
            [goodsList addObjectsFromArray:tmp];
        }
        self.goodsList = [goodsList copy];
    }
    [self.goodsList enumerateObjectsUsingBlock:^(NSArray<LXGoodBaseItemModel *> * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if(obj.count > 0) {
            LXGoodBaseItemModel *firstItem = obj.firstObject;
            if([firstItem isKindOfClass:[LXGoodBaseItemModel class]]) {
                if(firstItem.f_itemType == LXClassifyGoodItemTypeB2C ||
                   firstItem.f_itemType == LXClassifyGoodItemTypeO2O) {
                    __isEmpty = NO;
                    *stop = YES;
                    return;
                }
    ...
    if(self.rightModel.f_idxType == LXSubCategoryIndexTypeFirst) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.collectionView.mj_header.state = MJRefreshStateNoMoreData;
        });
...
- (void)selectItemAtIndex:(NSInteger)idx {
    ...
    if(idx < 0 || idx >= self.goodsList.count) {
        return;
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    LXGoodBaseItemModel *itemModel = self.goodsList[indexPath.section][indexPath.row];
    switch (itemModel.f_itemType) {
        case LXClassifyGoodItemTypeBanner: {
        ...
        case LXClassifyGoodItemTypeEmpty: case LXClassifyGoodItemTypeUnknown: {
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    if([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        if(indexPath.section == kPinCategoryViewSectionIndex) {
            ...
            LXClassifyBaseCategoryModel *sectionModel = self.rightModel.f_categorys[indexPath.section];
